...
namespace NPOItest.Controllers
{
    public class NPOIController : Controller
    {
        ...
        public void EmptyExport_XL()
        {
            FileStream fs = new FileStream(string.Concat(Server.MapPath(fileSavedPath), "/Excels/temp2.xlsx"), FileMode.Open, FileAccess.ReadWrite);
            XSSFWorkbook templateWorkbook = NPServices.AccountEmpty_XL(fs);

            Response.Clear();
            Response.ContentType = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet";
            Response.AddHeader("Content-Disposition", "attachment; filename=" + Server.UrlPathEncode("AccountEmpty.xlsx"));
            MemoryStream ms = new MemoryStream();
            templateWorkbook.Write(ms);
            ms.WriteTo(Response.OutputStream);
            //Response.Flush();
        ...
        public void DataExport_XL()
        {
            FileStream fs = new FileStream(string.Concat(Server.MapPath(fileSavedPath), "/Excels/temp2.xlsx"), FileMode.Open, FileAccess.ReadWrite);
            IWorkbook templateWorkbook = NPServices.AccountData_XL(fs);

            Response.Clear();
            Response.ContentType = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet";
            Response.AddHeader("Content-Disposition", "attachment; filename=" + Server.UrlPathEncode("AccountData.xlsx"));
            MemoryStream ms = new MemoryStream();
            templateWorkbook.Write(ms);
            ms.WriteTo(Response.OutputStream);
            Response.End();
